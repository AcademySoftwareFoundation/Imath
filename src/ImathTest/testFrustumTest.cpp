...
testFrustumTest ()
{
    ...
    IMATH_NAMESPACE::Matrix44<float> cameraMat;
    IMATH_NAMESPACE::Vec3<float> cameraPos (100.0f, 200.0f, 300.0f);
    cameraMat.makeIdentity ();
    cameraMat.translate (cameraPos);

    IMATH_NAMESPACE::FrustumTest<float> frustumTest (
        frustum, cameraMat);

    /////////////////////////////////////////////////////
    // Test Vec3's
    IMATH_NAMESPACE::Vec3<float> insideVec (
        100.0f, 200.0f, 300 - 2.0f);
    IMATH_NAMESPACE::Vec3<float> outsideVec_near (
        100.0f, 200.0f, 300 - 1.5f);
    IMATH_NAMESPACE::Vec3<float> outsideVec_far (
        100.0f, 200.0f, 300 - 568.0f);
    IMATH_NAMESPACE::Vec3<float> outsideVec_side (
        100.0f, 200.0f + 100.0f, 300 - 2.0f);
    IMATH_NAMESPACE::Vec3<float> outsideVec_up (
    ...
    /////////////////////////////////////////////////////
    // Test Boxes
    IMATH_NAMESPACE::Vec3<float> tinySize (0.0001f, 0.0001f, 0.0001f);
    IMATH_NAMESPACE::Vec3<float> hugeSize (1000.0f, 1000.0f, 1000.0f);

    // Empty box should NOT be visible
    assert (!frustumTest.isVisible (IMATH_NAMESPACE::Box<
                                    IMATH_NAMESPACE::Vec3<float>> ()));

    // Tiny box inside the frust should be visible
    assert (frustumTest.isVisible (
        IMATH_NAMESPACE::Box<IMATH_NAMESPACE::Vec3<float>> (
    ...
    assert (frustumTest.isVisible (
        IMATH_NAMESPACE::Sphere3<float> (insideVec, tinyRadius)));
    ...
    assert (frustumTest.isVisible (
        IMATH_NAMESPACE::Sphere3<float> (insideVec, hugeRadius)));
    assert (frustumTest.isVisible (IMATH_NAMESPACE::Sphere3<float> (
        outsideVec_near, hugeRadius)));
    assert (frustumTest.isVisible (
        IMATH_NAMESPACE::Sphere3<float> (outsideVec_far, hugeRadius)));
    assert (frustumTest.isVisible (IMATH_NAMESPACE::Sphere3<float> (
        outsideVec_side, hugeRadius)));
    assert (frustumTest.isVisible (
        IMATH_NAMESPACE::Sphere3<float> (outsideVec_up, hugeRadius)));

    // Tiny spheres outside should NOT be visible
    assert (!frustumTest.isVisible (IMATH_NAMESPACE::Sphere3<float> (
        outsideVec_near, tinyRadius)));
    assert (!frustumTest.isVisible (
        IMATH_NAMESPACE::Sphere3<float> (outsideVec_far, tinyRadius)));
    assert (!frustumTest.isVisible (IMATH_NAMESPACE::Sphere3<float> (
        outsideVec_side, tinyRadius)));
    assert (!frustumTest.isVisible (
        IMATH_NAMESPACE::Sphere3<float> (outsideVec_up, tinyRadius)));
    cout << "passed Sphere\n";
