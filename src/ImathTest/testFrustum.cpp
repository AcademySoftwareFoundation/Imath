...
void
testFrustumPlanes (IMATH_NAMESPACE::Frustumf& frustum)
{
    bool                          ortho = frustum.orthographic ();
    IMATH_NAMESPACE::V3f o (0.0f, 0.0f, 0.0f);
    float                         eps = 5.0e-4f;
                      ...
                      8.0f})
                {
                         ...
                         {-10.0f, -7.0f, -4.0f, -1.0f, 2.0f, 5.0f, 8.0f})
                    {
                        for (auto zTr: {-10.0f, -6.0f, -2.0f, 2.0f, 6.0f})
                        {
                            ...
                            IMATH_NAMESPACE::Eulerf e (
                                xRoRad, yRoRad, zRoRad);
                            IMATH_NAMESPACE::M44f mView =
                                e.toMatrix44 ();
                            mView.translate (
                                IMATH_NAMESPACE::V3f (xTr, yTr, zTr));

                            IMATH_NAMESPACE::Plane3f planes0[6];
                            frustum.planes (planes0);

                            IMATH_NAMESPACE::Plane3f planes[6];
                            frustum.planes (planes, mView);

                            IMATH_NAMESPACE::V3f up =
                                IMATH_NAMESPACE::V3f (0, 1, 0);
                            assert ((up ^ planes0[0].normal) > 0.0);
                            mView.multDirMatrix (up, up);
                            assert ((up ^ planes[0].normal) > 0.0);

                            IMATH_NAMESPACE::V3f pt =
                                (!ortho) ? o
                                         : IMATH_NAMESPACE::V3f (
                                               0.0f, frustum.top (), 0.0f);
                            float d = planes0[0].distanceTo (pt);
                            assert (IMATH_NAMESPACE::iszero (d, eps));
                            pt = pt * mView;
                            d  = planes[0].distanceTo (pt);
                            assert (IMATH_NAMESPACE::iszero (d, eps));

                            IMATH_NAMESPACE::V3f right =
                                IMATH_NAMESPACE::V3f (1, 0, 0);
...
testFrustum ()
{
    ...
    IMATH_NAMESPACE::Frustum<float> frustum (n, f, l, r, t, b, false);

    assert (
        IMATH_NAMESPACE::abs<float> (
    ...
    IMATH_NAMESPACE::M44f m = frustum.projectionMatrixExc ();
    assert (
        IMATH_NAMESPACE::abs<float> (m[0][0] - ((2 * n) / (r - l))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[0][1]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[0][2]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[0][3]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][0]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][1] - ((2 * n) / (t - b))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][2]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][3]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][0] - ((r + l) / (r - l))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][1] - ((t + b) / (t - b))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][2] - (-(f + n) / (f - n))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][3] - -1.0f) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][0]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][1]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (
            m[3][2] - ((-2 * f * n) / (f - n))) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][3]) < 1e-6f);
    ...
    assert (
        ...
        IMATH_NAMESPACE::abs<float> (m[1][1] - ((2 * n) / (t - b))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][2]) < 1e-6 &&
        IMATH_NAMESPACE::abs<float> (m[1][3]) < 1e-6 &&
        IMATH_NAMESPACE::abs<float> (m[2][0] - ((r + l) / (r - l))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][1] - ((t + b) / (t - b))) <
            1e-6f &&
    ...
    IMATH_NAMESPACE::Frustum<float> badFrustum;
    ...
    m = frustum.projectionMatrix ();
    assert (
        IMATH_NAMESPACE::abs<float> (m[0][0] - (2 / (r - l))) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[0][1]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[0][2]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[0][3]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][0]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][1] - (2 / (t - b))) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][2]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[1][3]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][0]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][1]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][2] - (-2 / (f - n))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[2][3]) < 1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][0] - (-(r + l) / (r - l))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][1] - (-(t + b) / (t - b))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][2] - (-(f + n) / (f - n))) <
            1e-6f &&
        IMATH_NAMESPACE::abs<float> (m[3][3] - 1.0f) < 1e-6f);
    cout << "1";

    cout << "\nplanes ";
    testFrustumPlanes (frustum);

    // TODO - There are many little functions in IMATH_NAMESPACE::Frustum which
    // aren't tested here.  Those test should be added.  But this is
    // a start.

    IMATH_NAMESPACE::Frustum<float> f1 (n, f, l, r, t, b, false);
    IMATH_NAMESPACE::Frustum<float> f2 (n, f, l, r, t, b, true);
    assert (f1 != f2);
    ...
    float                                 zero = 0;
    float                                 one  = 1;
    IMATH_NAMESPACE::Vec3<float> v3 (zero, zero, one);
